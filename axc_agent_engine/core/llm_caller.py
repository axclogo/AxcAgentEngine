...
class StreamEventEmitter:
	...
	def __init__(self, ctx: ExecutionContext) -> None:
		self._ctx = ctx
		self._event_queue: asyncio.Queue | None = ctx.runtime.event_queue
	...
	async def finish_thinking(self, content: str) -> None:
		if self._in_thinking and self._event_queue:
			await self._event_queue.put(Event(type=EventType.THINKING_END, content=content))
		self._in_thinking = False
	...
	async def _thinking_delta(self, content: str) -> None:
		...
		if not self._in_thinking:
			...
			await self._event_queue.put(Event(type=EventType.THINKING_START))
		await self._event_queue.put(Event(type=EventType.THINKING_DELTA, content=content))
	...
	async def _content_delta(self, content: str) -> None:
		...
		if self._in_thinking:
			self._in_thinking = False
			await self._event_queue.put(Event(type=EventType.THINKING_END))
		if not self._stream_started:
			self._stream_started = True
			await self._event_queue.put(Event(type=EventType.STREAM_START))
		await self._event_queue.put(Event(type=EventType.STREAM_DELTA, content=content))
...
class StreamUsageReporter:
	...
	def apply_usage(self, ctx: ExecutionContext, result) -> list[Event]:
		...
		if result.cached_tokens > 0:
			events.append(Event(type=EventType.CACHE_HIT, metadata={"cached_tokens": result.cached_tokens}))
		if result.usage_input or result.usage_output:
...
class LLMCaller:
	...
	async def call(
		...
		if ctx.config.stream:
			result = await self._call_stream(ctx, messages, tools)
		else:
	...
	async def _call_stream(
		...
		for attempt in range(2):
			try:
				return await self._aggregate_stream(ctx, self._primary, messages, tools, **kwargs)
			except (httpx.NetworkError, httpx.TimeoutException, RetryableProviderError, LLMTimeoutError) as e:
				...
				if self._fallback:
					logger.warning(f"Primary LLM retry failed, switching to fallback: {e}")
					self._mark_fallback(ctx, str(e))
					return await self._aggregate_stream(ctx, self._fallback, messages, tools, **kwargs)
				raise
	...
	async def _aggregate_stream(
		...
		events: list[Event] = []
		if not ctx.runtime.event_queue:
	...
	def _process_sync_response(
		...
		return llm_resp.message.to_dict(), []
