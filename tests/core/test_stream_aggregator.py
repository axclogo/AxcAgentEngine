...
class TestStreamAggregator:
	...
	@pytest.mark.asyncio
	async def test_cache_metadata_is_preserved(self):
		chunks = [
			LLMStreamChunk(
				usage=LLMUsage(input_tokens=100, output_tokens=0, cached_tokens=50),
				metadata={"cache_type": "write", "tokens": 50},
			),
		]
		agg = StreamAggregator()
		result = await agg.aggregate(_make_stream(chunks), idle_timeout=10)
		assert result.usage_metadata["cache_type"] == "write"
		assert result.usage_metadata["tokens"] == 50
