...
class TestSessionManager:
	...
	@pytest.mark.asyncio
	async def test_restore_context_interrupt(self):
		sm = SessionManager()
		session = Session(session_id="s1")
		session.metadata["snapshot"] = {"status": "interrupted", "interrupt_reason": "timeout", "current_round": 5}
		ms = MessageStore()
		sm.restore_context(session, ms)
		all_msgs = ms.get_all()
		assert any("中断" in m.get("content", "") for m in all_msgs)
